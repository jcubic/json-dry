...
function findClass(value) {
	...
	if (typeof value == 'string') {
		...
		if (Dry.Classes[value]) {
			return Dry.Classes[value];
		}
	...
	if (value.path) {
		return fromPath(Dry.Classes, value.path);
	} else {
		if (value.namespace) {
			constructor = fromPath(Dry.Classes, value.namespace);
		} else {
			constructor = Dry.Classes;
		}
			...
			if (current.length) {
				...
				if (retrieve.hasOwnProperty(current)) {
					...
					if (typeof temp == 'undefined') {
						temp = retrieve[current] = retrieveFromPath(old, current.split(special_char));
					}
				}
...
function parse(object, reviver) {
	...
	undry_paths.forEach(function eachEntry(entry, path) {
		...
		setPath(old, path_array, entry, true);
...
var Dry = {
  stringify: stringify,
  toDryObject: toDryObject,
  parse: parse,
  clone: clone,
  Classes: {},
  registerClass: registerClass,
  registerUndrier: registerUndrier,
  registerDrier: registerDrier
};
